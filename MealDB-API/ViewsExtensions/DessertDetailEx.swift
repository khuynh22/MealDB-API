...
extension DessertDetailView {
    @MainActor class ViewModel: ObservableObject {
        ...
        func fetchMealDetails() async {
            func handleSuccess(_ items: NetworkResponseResult) -> Void {
        ...
        private func setInstructions(using details: NetworkResponse) {
            self.instructions = details.strInstructions
        }
        ...
        private func setIngredients(using details: NetworkResponse) {
            ...
            self.ingredients = ingredMeasurements.filter {
                !($0.name.removeEmpty() || $0.quantity.removeEmpty())
            }
        }
    }
}
